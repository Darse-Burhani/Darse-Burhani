/**
 * Lightweight Code-128B SVG Barcode Generator
 * Generates crisp, standard Code-128 SVG barcodes for library books without external library dependencies.
 */

// Code-128 pattern dictionary for ASCII characters 32-126
...
export function generateCode128Svg(text: string, height = 50, barWidth = 2): string {
  ...
  // Convert pattern digits (bar and space widths) to SVG rects
  let x = 10;
  let svgPaths = "";
  let isBar = true;
  ...
  for (let i = 0; i < patternStr.length; i++) {
    ...
    if (isBar) {
      svgPaths += `<rect x="${x}" y="4" width="${width}" height="${height}" fill="currentColor" />`;
    }
    x += width;
    isBar = !isBar;
  }
  ...
  return `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 ${totalWidth} ${totalHeight}" width="100%" height="${totalHeight}" preserveAspectRatio="xMidYMid meet" class="text-slate-900">
    <rect width="${totalWidth}" height="${totalHeight}" fill="#ffffff" rx="4" />
    ${svgPaths}
    <text x="${totalWidth / 2}" y="${totalHeight - 4}" text-anchor="middle" font-family="monospace" font-size="11" font-weight="700" fill="#0f172a">${clean}</text>
  </svg>`;
}
